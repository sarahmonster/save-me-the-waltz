 ...
 */
function savemethewaltz_is_frontpage() {
 ...
 */
function savemethewaltz_panel_count() {
 ...
 * @return array
 */
function savemethewaltz_body_classes( $classes ) {
 ...
 * @return array
 */
function savemethewaltz_post_classes( $classes ) {
	...
	if ( get_page_template_slug() && '' !== get_page_template_slug() ) :
		...
		if ( '' !== locate_template( $simple_slug ) ) :
			...
			$classes[] = 'savemethewaltz-' . $simple_slug;
		endif;
	endif;

	// Add a page template slug to the main posts page
	if ( get_the_ID() === (int) get_option( 'page_for_posts' )  ) :
		$classes[] = 'savemethewaltz-template-recent-posts';
	endif;

	// Add the panel classes to all posts in archive views
	if ( is_home() || is_search() || is_archive() ) :
		$classes[] = 'savemethewaltz-panel';
	endif;

	return $classes;
}
add_filter( 'post_class', 'savemethewaltz_post_classes' );

/*
 * Remove an element from an array.
 * Used in our post_class filtering machinations above.
 */
function savemethewaltz_remove_from_array( $item, $array ) {
 ...
 */
class savemethewaltz_Menu extends Walker_Nav_Menu {
 ...
 */
function savemethewaltz_custom_excerpt_length( $length ) {
	return 27;
}
add_filter( 'excerpt_length', 'savemethewaltz_custom_excerpt_length', 999 );

function savemethewaltz_custom_excerpt_more($more) {
	$more_link = '<span class="read-more">';
	$more_link .= '<a class="more-link" href="' . esc_url( get_the_permalink() ) . '" rel="bookmark">';
	$more_link .= sprintf(
		wp_kses( __( 'Read more %s', 'save-me-the-waltz' ), array( 'span' => array( 'class' => array() ) ) ),
		the_title( '<span class="screen-reader-text">"', '"</span>', false )
	);
	$more_link .= '</a></span>';
	return '&hellip; ' . $more_link;
}
add_filter( 'excerpt_more', 'savemethewaltz_custom_excerpt_more' );

/*
 * Filter the categories archive widget to add a span around post count
 */
function savemethewaltz_cat_count_span( $links ) {
	$links = str_replace( '</a> (', '</a><span class="post-count">(', $links );
	$links = str_replace( ')', ')</span>', $links );
	return $links;
}
add_filter( 'wp_list_categories', 'savemethewaltz_cat_count_span' );

/*
 * Filter the archives widget to add a span around post count
 */
function savemethewaltz_archive_count_span( $links ) {
	$links = str_replace( '</a>&nbsp;(', '</a><span class="post-count">(', $links );
	$links = str_replace( ')', ')</span>', $links );
	return $links;
}
add_filter( 'get_archives_link', 'savemethewaltz_archive_count_span' );
